import { randomUUID } from "expo-crypto";
import type { SQLiteDatabase } from "expo-sqlite";
import type { CoverageCatalog, CoverageStateRepository, ProgressRepository } from "../../core/contracts";
import type { EdgeExclusion, TrackingSession, TrackPoint } from "../../core/types";
import { edgeIsComplete, progressPercent } from "../../modules/coverage/progress";
import { LocalCoverageStateReader } from "./LocalCoverageStateReader";
import { openAppDatabase } from "./openAppDatabase";

export class LocalProgressRepository implements ProgressRepository, CoverageStateRepository {
  private readonly coverageState: LocalCoverageStateReader;

  private constructor(
    private readonly database: SQLiteDatabase,
    private readonly catalog: CoverageCatalog,
    private readonly cityId: string,
    private readonly regionVersion: string,
  ) {
    this.coverageState = new LocalCoverageStateReader(database, catalog, cityId, regionVersion);
  }

  static async open(catalog: CoverageCatalog, cityId: string, regionVersion: string) {
    return new LocalProgressRepository(await openAppDatabase(), catalog, cityId, regionVersion);
  }

  async recoverInterruptedSessions() {
    await this.database.runAsync(
      "UPDATE sessions SET status='interrupted', ended_at=COALESCE(ended_at, ?) WHERE status='active'",
      Date.now(),
    );
  }

  async createSession(): Promise<TrackingSession> {
    const session: TrackingSession = {
      id: randomUUID(),
      cityId: this.cityId,
      regionVersion: this.regionVersion,
      startedAt: Date.now(),
      endedAt: null,
      status: "active",
    };
    await this.database.runAsync(
      "INSERT INTO sessions(id, city_id, region_version, started_at, status) VALUES (?, ?, ?, ?, ?)",
      session.id,
      session.cityId,
      session.regionVersion,
      session.startedAt,
      session.status,
    );
    return session;
  }

  async finishSession(id: string, status: TrackingSession["status"]) {
    await this.database.runAsync("UPDATE sessions SET ended_at=?, status=? WHERE id=?", Date.now(), status, id);
  }

  async appendTrack(points: readonly TrackPoint[]) {
    await this.database.withTransactionAsync(async () => {
      for (const point of points) {
        await this.database.runAsync(
          `INSERT INTO track_points(session_id, recorded_at, longitude, latitude, accuracy, speed, heading)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          point.sessionId,
          point.recordedAt,
          point.coordinate[0],
          point.coordinate[1],
          point.accuracyM,
          point.speedMps,
          point.headingDeg,
        );
      }
    });
  }

  async saveVisitedSamples(ids: readonly string[]) {
    if (!ids.length) return;
    const samples = await this.catalog.resolveSamples(ids);
    const edges = new Map(samples.map((sample) => [sample.edgeId, sample]));
    await this.database.withTransactionAsync(async () => {
      for (const sample of samples) {
        await this.database.runAsync(
          "INSERT OR IGNORE INTO visited_samples VALUES (?, ?, ?, ?)",
          this.cityId,
          this.regionVersion,
          sample.sampleId,
          sample.edgeId,
        );
      }
      for (const sample of edges.values()) await this.refreshEdge(sample);
    });
  }

  private async refreshEdge(sample: Awaited<ReturnType<CoverageCatalog["resolveSamples"]>>[number]) {
    const row = await this.database.getFirstAsync<{ count: number }>(
      "SELECT count(*) count FROM visited_samples WHERE city_id=? AND region_version=? AND edge_id=?",
      this.cityId,
      this.regionVersion,
      sample.edgeId,
    );
    const count = row?.count ?? 0;
    await this.database.runAsync(
      "INSERT OR REPLACE INTO edge_progress VALUES (?, ?, ?, ?, ?, ?)",
      this.cityId,
      this.regionVersion,
      sample.edgeId,
      count,
      sample.sampleCount,
      edgeIsComplete(count, sample.sampleCount) ? 1 : 0,
    );
  }

  async getCityProgress() {
    const total = await this.catalog.countEligibleEdges(this.cityId);
    const excluded = await this.scalar("SELECT count(*) count FROM exclusions WHERE city_id=?", this.cityId);
    const completed = await this.scalar(
      `SELECT count(*) count FROM edge_progress p WHERE p.city_id=? AND p.region_version=? AND p.completed=1
       AND NOT EXISTS(SELECT 1 FROM exclusions x WHERE x.city_id=p.city_id AND x.edge_id=p.edge_id)`,
      this.cityId,
      this.regionVersion,
    );
    const eligibleEdges = Math.max(0, total - excluded);
    return { cityId: this.cityId, completedEdges: completed, eligibleEdges, excludedEdges: excluded, percent: progressPercent(completed, eligibleEdges) };
  }

  private async scalar(sql: string, ...params: (string | number)[]) {
    return (await this.database.getFirstAsync<{ count: number }>(sql, ...params))?.count ?? 0;
  }

  async exclude(exclusion: EdgeExclusion) {
    if (await this.catalog.edgeCity(exclusion.edgeId) !== this.cityId || !(await this.catalog.edgeEligible(exclusion.edgeId))) {
      throw new Error("Ineligible edge");
    }
    await this.database.runAsync(
      "INSERT OR REPLACE INTO exclusions VALUES (?, ?, ?, ?)",
      this.cityId,
      exclusion.edgeId,
      exclusion.reason,
      exclusion.createdAt,
    );
  }

  async undoExclusion(edgeId: string) {
    await this.database.runAsync("DELETE FROM exclusions WHERE city_id=? AND edge_id=?", this.cityId, edgeId);
  }

  getEdgeStates(ids?: readonly string[]) {
    return this.coverageState.getEdgeStates(ids);
  }

  getCoverageSegments(ids?: readonly string[]) {
    return this.coverageState.getCoverageSegments(ids);
  }
}
